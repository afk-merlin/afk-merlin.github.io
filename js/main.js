...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (preloader) {
        const msgs = ['Engaging ROOM...', 'Calibrating gyros...', 'Linking telemetry...', 'Cleared for takeoff ☠️'];
        let prog = 0, mi = 0;
    ...
    if (particlesContainer && window.innerWidth > 768) {
        for (let i = 0; i < 30; i++) {
            ...
            p.style.width = s + 'px';
            p.style.height = s + 'px';
            const colors = ['#22d3ee','#a5f3fc','#38bdf8','#facc15','#0e7490','#7dd3fc'];
        ...
        for (let i = 0; i < 50; i++) {
            ...
            star.style.background = Math.random() > 0.7 ? '#22d3ee' : '#dbeafe';
    ...
    if (tw) {
        const phrases = [
            ' ROOM. everything in range is mine ☠️',
            ' FPV builds — calm hands, clean cuts',
            ' multirotor airframes, field-ready 🛠️',
            ' LoRa mesh where the grid ends 📡',
            ' telemetry locked, eyes in the sky 🛰️',
            ' firmware that behaves under fire ⚡',
            ' free skies. Worst Generation. 🏴‍☠️',
        ];
    ...
    document.addEventListener('click', e => {
        for (let i = 0; i < 5; i++) {
            ...
            spark.style.cssText = `
                ...
                background:${['#22d3ee','#a5f3fc','#facc15'][Math.floor(Math.random()*3)]};
